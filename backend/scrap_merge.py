...
# Spotify API setup
SPOTIFY_CLIENT_ID = '9db66a8a93514a43baa61db7d3880242'
SPOTIFY_CLIENT_SECRET = '221735d938054591b54b2feda1074b80'
...
def load_and_merge_datasets(dataset_path: str, csv_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        lyrics_df = lyrics_df[['artist', 'song']].rename(columns={'song': 'track_name'})
        
        # Combine and deduplicate
        combined_df = pd.concat([spotify_df, lyrics_df], ignore_index=True)
...
def get_song_lyrics(song_name: str, artist_name: str, retries: int = 3) -> str:
    """Get lyrics from Genius with caching and retries"""
...
def fetch_lyrics_row(row: pd.Series) -> Dict[str, Any]:
    """Fetch lyrics for a single row"""
    lyrics = get_song_lyrics(row['track_name'], row['artist'])
    return {
        'song': row['track_name'],
        'artist': row['artist'],
        'lyrics': lyrics
...
def process_lyrics(missing_songs: pd.DataFrame, max_workers: int = 1000) -> pd.DataFrame:
    """Process lyrics for multiple songs using ThreadPoolExecutor"""
    lyrics_data = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(fetch_lyrics_row, row): idx for idx, row in missing_songs.iterrows()}
...
def process_spotify_data(lyrics_df: pd.DataFrame) -> pd.DataFrame:
    """Process Spotify data for songs with lyrics"""
    spotify_data = []
    
    for _, row in tqdm(lyrics_df.iterrows(), total=len(lyrics_df), desc="Processing Spotify data"):
        # Check cache first
...
def main():
    ...
    try:
        merged_df = load_and_merge_datasets(dataset_path, lyrics_path)
    except Exception as e:
    ...
    lyrics_df = process_lyrics(merged_df)


    print("\nFetching Spotify data for songs with lyrics...")
    spotify_df = process_spotify_data(lyrics_df)
